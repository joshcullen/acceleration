...
In this comparison, Slow-Uniform behavior was more likely to occur in habitats with greater vegetation productivity (greenness), but the Slow-Turn state was more likely to occur at greater elevations. Wetness did not appear to significantly impact the probability of exhibiting either state.
...
Similar to the previous comparison, increasing values in greenness were associated with a greater probability of exhibiting an Exploratory state, while higher elevations were more indicative of a Slow-Turn state. Again, wetness did not drive the probability of exhibiting either state.
...
Nearly identical to the previous two comparisons, Transit was more likely to occur with greater levels of vegetation productivity whereas Slow-Turn was more likely at greater elevations. Wetness also did not appear to have a significant effect on the probability of either state.
...
In this comparison, none of the 3 covariates significantly impacted the probability of exhibiting either state. This relationship differs largely from that observed when comparing against the Slow-Turn state.
...
Although the 95% confidence intervals for greenness and elevation slightly crossed zero, the coefficient estimates appeared similar to previous comparisons. That is to say, the faster state (Transit) was more likely with increasing vegetation productivity, but the slower state (Slow-Uniform) was more likely at greater elevations.
...
Similar to the previous comparisons, all 95% confidence intervals for the coefficients crossed a value of one, but greenness and elevation appeared to reflect previous trends of faster movements in highly productive habitat and slow movements at higher elevations. Wetness did not significantly impact the probability of exhibiting either state.
...
When assessing the effects of environmental covariates on the probability of exhibiting one state over another, there was typically a consistent pattern when moving from slow to fast states. In each of the logistic regressions, the slower state was often favored when there were greater elevations, whereas the faster state was favored with higher values of greenness. The wetness covariate did not appear to significantly impact the probability of exhibiting any state over another.\newline
